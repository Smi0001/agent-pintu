...
function resolveSettings(args) {
  ...
  const explicitMode = flags.mode ? String(flags.mode) : (config.mode || null);
  const mode = explicitMode && ['routes', 'touched'].includes(explicitMode)
    ? explicitMode
    : null;
    ...
    options: [
      {
        value: 'routes',
        label: 'routes',
        hint: 'Walk from changed Express route handlers (backend servers)',
      },
      {
        value: 'touched',
        label: 'touched',
        hint: 'Walk from every function touched by the diff (any code)',
      },
    ],
    initialValue: 'routes',
  });
...
async function runTrace(args) {
  ...
  const mode = await selectMode(s.mode);
  const git = makeGit(s.project);

  if (!fs.existsSync(path.join(s.project, '.git'))) {
    throw new Error(`Not a git repo: ${s.project}`);
  }
  if (s.tsConfigPath && !fs.existsSync(s.tsConfigPath)) {
    throw new Error(`tsconfig not found: ${s.tsConfigPath} (override with --ts-config or remove from .pintu.json)`);
  }
  ...
  console.error(`[pintu] mode:         ${mode}`);
  console.error(`[pintu] project:      ${s.project}`);
  console.error(`[pintu] server root:  ${s.serverRoot}`);
  console.error(`[pintu] tsconfig:     ${s.tsConfigPath || '(none — using synthesized JS/TS project)'}`);
  ...
  const serverFiles = all.filter((f) => f.startsWith(serverPrefix) && SOURCE_EXT_RE.test(f));
  const routeFiles = serverFiles.filter((f) => f.includes(s.routesPattern));

  console.error(
    `[pintu] changed: ${all.length} files (${serverFiles.length} source, ${routeFiles.length} route files)`,
  );

  const tsProject = s.tsConfigPath
    ? loadFromTsConfig(s.tsConfigPath)
    : synthesizeProject(s.serverRoot);
  ...
  let entries;
  if (mode === 'touched') {
    entries = findTouchedEntries(tsProject, s.project, serverFiles, fileHunks);
    if (!entries.length) {
      throw new Error('No touched top-level functions found in changed files.');
    }
    console.error('[pintu] touched entries:');
    for (const e of entries) {
      console.error(
        `  ${e.method.padEnd(6)} ${e.routePath.padEnd(50)} → ${e.handlerName}  (${e.routeFile})`,
      );
    }
  } else {
    entries = findRouteEntries(tsProject, s.project, routeFiles);
    if (!entries.length) {
      const hint = serverFiles.length
        ? `Hint: this PR changed ${serverFiles.length} source file(s) but no Express routes were detected. Try --mode touched.`
        : `Hint: no source files in the diff. This PR may only change non-code files (HTML, SQL, etc.).`;
