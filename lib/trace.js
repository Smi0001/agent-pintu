...
function machineYaml(meta) {
    ...
    for (const rr of rootResults) {
      ...
      if (rr.entries.length) {
        ...
        for (const e of rr.entries) {
          ...
          // Fall back to routeFile when no path is available — earlier
          // versions emitted "undefined" as a literal here for FN entries
          // and other non-route cases, which made the per-PR MD/YAML
          // unreadable. Filename keeps the entry identifiable.
          lines.push(`        path: "${e.displayPath || e.routePath || e.fullPath || e.routeFile || ''}"`);
  ...
  for (const rr of rootResults) {
    for (const e of rr.entries) {
      lines.push(`  - method: ${e.method}`);
      // Same filename fallback as the multi-root entries block above.
      lines.push(`    path: "${e.displayPath || e.routePath || e.fullPath || e.routeFile || ''}"`);
      lines.push(`    handler: ${JSON.stringify(e.handlerName)}`);
      lines.push(`    route_file: ${JSON.stringify(e.routeFile)}`);
...
function renderMarkdown({ title, base, nodes, edges, rootResults, project, isMultiRoot }) {
  ...
  for (const rr of rootResults) {
    ...
    for (const pe of rr.entries) {
      // For FN entries: the handler name is the meaningful label (no
      // route path exists). For route entries: prefer the visible path,
      // but fall back to routeFile when displayPath/fullPath/routePath
      // all resolve empty — better than rendering "## GET undefined".
      const routeLabel =
        pe.displayPath || pe.fullPath || pe.routePath || pe.routeFile || '';
      const heading = pe.method === 'FN'
        ? `${isMultiRoot ? '### ' : '## '}${pe.handlerName}`
        : `${isMultiRoot ? '### ' : '## '}${pe.method} ${routeLabel}`;
      md += `${heading}\n\n`;
